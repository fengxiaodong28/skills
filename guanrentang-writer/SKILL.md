...
**步骤 1**：获取火山方舟 API Key
- 访问 [火山方舟开放平台](https://console.volcengine.com/ark/) 注册并获取 API Key
...
| `biyantie-detail.jpg` | 鼻炎贴详情图 | 鼻宝宝鼻炎贴产品详情/使用说明图 |
...
| 位置 | 推荐尺寸 | 比例 | Seedream size | 备注 |
|------|---------|------|---------------|------|
| 正文配图 | 1080×810 px | 4:3 | `2560x1920` | AI 生成 |
| 食疗图 | 1080×1080 px | 1:1 | `1920x1920` | AI 生成 |
| 开头装饰图 | - | - | - | **固定素材** |
| 结尾系列图 | - | - | - | **固定素材**（4张） |

> **注意**：doubao-seedream-5-0 尺寸总像素需 ≥ 3,686,400
...
| 图片类型 | 尺寸 | Prompt 模板 |
|---------|------|------------|
| 配图 | `2560x1920` | `中医养生插画，{描述内容}，新中式水墨插画风格，中国传统色（黛青、朱砂、米白），淡雅晕染效果，留白构图，温暖治愈氛围，适合微信公众号配图` |
| 食疗 | `1920x1920` | `中式养生美食摄影，{食疗方名称}，陶瓷或木质器皿盛放，点缀中药材装饰，清新淡雅，养生氛围，暖色调，高清细节` |
...
curl -s -X POST "https://ark.cn-beijing.volces.com/api/v3/images/generations" \
  -H "Authorization: Bearer $ZHIPU_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"doubao-seedream-5-0-260128","prompt":"中医养生插画，春季养肝疏肝示意图，肝脏与人体气机运行，新中式水墨插画风格，中国传统色（黛青、朱砂、米白），淡雅晕染效果，留白构图，温暖治愈氛围","sequential_image_generation":"disabled","response_format":"url","size":"2560x1920","stream":false,"watermark":true}' \
  | jq -r '.data[0].url' \
...
|---------|----------|
| 开头装饰 | **固定素材**：header-decorative.jpg |
| 产品图1 | **固定素材**：biyantie-detail.jpg（产品详情/使用说明图） |
...
![鼻宝宝鼻炎贴使用说明](./assets/biyantie-detail.jpg)
